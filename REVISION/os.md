...
The Banker's Algorithm checks, **before granting a request**, whether the system stays in a **safe state** (some order exists where every process can still finish). If yes → grant. If no → make the process wait.

### Setup

For each process track three matrices/vectors:

```text
Allocation[i] → resources currently held by process i
Max[i]        → maximum resources process i may ever need
Need[i]       → Max[i] − Allocation[i]  (still might request)
Available     → free resources system currently has
```
...
Allocated total = (7, 2, 5)
Available       = Total − Allocated = (3, 3, 2)
```

### Safety Check — find a safe sequence

At each step, find a process whose **Need ≤ Available**. Let it finish, add its Allocation back to Available, repeat.

```text
Step 1: Available = (3,3,2)
  Check P0: Need(7,4,3) → NOT ≤ Available → skip
  Check P1: Need(1,2,2) → ≤ (3,3,2) ✅ → run P1, finishes
            Available += Allocation(P1) = (3,3,2)+(2,0,0) = (5,3,2)

Step 2: Available = (5,3,2)
  Check P0: Need(7,4,3) → NOT ≤ → skip
  Check P2: Need(6,0,0) → NOT ≤ → skip
  Check P3: Need(0,1,1) → ≤ (5,3,2) ✅ → run P3, finishes
            Available += Allocation(P3) = (5,3,2)+(2,1,1) = (7,4,3)

Step 3: Available = (7,4,3)
  Check P0: Need(7,4,3) → ≤ (7,4,3) ✅ → run P0, finishes
            Available += Allocation(P0) = (7,4,3)+(0,1,0) = (7,5,3)

Step 4: Available = (7,5,3)
  Check P2: Need(6,0,0) → ≤ ✅ → run P2, finishes
            Available += (3,0,2) = (10,5,5)

Step 5: Available = (10,5,5)
  Check P4: Need(4,3,1) → ≤ ✅ → run P4, finishes
```

**Safe sequence found:** `P1 → P3 → P0 → P2 → P4` → system is in a **safe state**.

### Handling a new request

If process Pi requests resources `Request[i]`:

```text
1. If Request[i] ≤ Need[i]        else → error, exceeds max claim
2. If Request[i] ≤ Available      else → wait, not enough free resources
3. Pretend to allocate:
     Available -= Request[i]
     Allocation[i] += Request[i]
     Need[i] -= Request[i]
4. Run the safety check again on this pretend state.
     Safe  → grant for real.
     Unsafe → roll back, process must wait.
```

**Trick to remember:** Banker's Algorithm = a bank manager who only lends money if, worst case, every customer could still eventually be paid back in some order.
...
* **Safe State** → a sequence exists where every process can finish with available + released resources.
* **Need Matrix** → Max − Allocation; what a process might still request.
